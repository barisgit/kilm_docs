...
5.  **Processes Jinja:** Renders filenames and file content using Jinja2, substituting the final collected variables.
...
```yaml
description: Default 4 layer KiCad project template
name: default-4layer
...
```text
{{ directory_name }}/  # Root directory named using variables
├── docs/
├── libraries/
├── production/
├── {{ project_name|replace(' ', '') }}_{{ subproject_name|replace(' ', '') }}.kicad_pro
├── {{ project_name|replace(' ', '') }}_{{ subproject_name|replace(' ', '') }}.kicad_sch
├── {{ project_name|replace(' ', '') }}_{{ subproject_name|replace(' ', '') }}.kicad_pcb
├── .gitignore
└── README.md
```

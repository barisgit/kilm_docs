...
<Aside type="caution">
**Important:** KiLM creates timestamped backups of KiCad's configuration files (e.g., `fp-lib-table.backup.YYYYMMDD_HHMMSS`, `sym-lib-table.backup.YYYYMMDD_HHMMSS`, `kicad_common.json.backup.YYYYMMDD_HHMMSS`) in the *same* directory as the original files *before* making any changes. The number of backups retained is controlled by the `max_backups` setting in `config.yaml`. This allows you to easily revert if something goes wrong.
</Aside> 